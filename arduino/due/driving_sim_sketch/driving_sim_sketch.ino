...
void setup() {
  ...
  cal.wheel_power_min = -70;
  cal.wheel_power_max = 70;
...
void loop() {
  ...
  if(fsr1_raw >= force_threshold || fsr2_raw >= force_threshold) 
  {
    ...
    {
      setWheelPower(-Kf * (fsr1_raw - force_threshold));
    }
    else if (fsr2_raw > fsr1_raw) //turning left -> assist left
    {
      setWheelPower(Kf * (fsr2_raw - force_threshold)) 
    }
    else
    {
      setWheelPower(0); //Indeterminate State
    }
  }  
  else
  {
    if(wheel_angle.d > 0 || wheel_angle.d < 0) 
    {
